...
export default function WritePage() {
  ...
  const [articleTitleValue, setArticleTitleValue] = useLocalStorage(
    'articleTitle',
    ''
  );
  const [articleDescriptionValue, setArticleDescriptionValue] = useLocalStorage(
    'articleDescription',
    ''
  );
  const [articleContent, setArticleContent] = useLocalStorage(
    'articleContent',
    ''
  );
  ...
  const createArticle = async () => {
    try {
      ...
      window.localStorage.removeItem('articleDescription');
      window.localStorage.removeItem('articleContent');
      window.localStorage.removeItem('selectedArticleCategory');
    } catch (error) {
      toast.error('Failed to create an article. Try again', { toastId: '450' });
    }
  };
  ...
  useEffect(() => {
    if (!user) {
      navigate('/');
      toast.error('You need to sign in to write an article');
    }
  }, []);
  ...
  return (
    <>
      ...
      />
      <Helmet>
        <title>Write an article - Blog App</title>
      </Helmet>
      {user ? (
        <div className={styles['write-page']}>
          <h1 className={styles['write-page__title']}>New article</h1>
          {!preview ? (
            <div
